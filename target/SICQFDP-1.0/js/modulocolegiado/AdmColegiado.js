...
$(document).ready(function(){
    ...
    $("#cboIngresoInscripcion").focus();
    $("#cboIngresoInscripcion").on("focusout",function(){
       $("#panelError").html("");  
       var opcion = parseInt($("#cboIngresoInscripcion").val());
       if (isNaN(opcion) || opcion === 0){
           mostrarMensaje('danger', '[Error: Debe seleccionar si es una Inscripcion o un Ingreso de colegiatura]');
           $("#cboIngresoInscripcion").focus();
       } else {
           $("#cboIngresoInscripcion").prop('disabled', true);
           $("#txtDni").focus();
       }
    });
...
//    });
    ...
    $("#OpcCancelar").on("click", function(){
       ...
       $("#cboIngresoInscripcion").val("0");
       $("#cboIngresoInscripcion").prop("disabled", false);
       $("#cboIngresoInscripcion").focus();
    });
...
function buscarPersonaColegiado(){
    ...
    var esIngresoInscripcion = parseInt($("#cboIngresoInscripcion").val());
    if (isNaN(esIngresoInscripcion) || esIngresoInscripcion === 0){
        mostrarMensaje('danger', '[Error: Debe seleccionar si es una Inscripcion'
                +' o un Ingreso de colegiatura]');
        $("#cboIngresoInscripcion").focus();
        return;
    }
    ...
    if (esIngresoInscripcion === 1){
        colegiadoRep = colegiado.buscarPersonaColegiadoInscripIngre(1);
    } else {
        colegiadoRep = colegiado.buscarPersonaColegiadoInscripIngre(2);
    }

    if (colegiadoRep.indError === 0) {
        if(colegiadoRep.esColegiado === 1){
            llenarFormularioColegiado(colegiadoRep);
            deshabilitarDatosColegiado(true);
...
function grabarIngresoInscripcion(){
    var indIncripIngre, dni, numColegiatura, fechaInscripIngre;
    ...
    indIncripIngre = parseInt($("#cboIngresoInscripcion").val());
    if(isNaN(indIncripIngre) || indIncripIngre === 0){
        mostrarMensaje('danger', 'Error: Debe seleccionar si es una Inscripcion o'
                +' Ingreso de Colegiatura');
        $("#cboIngresoInscripcion").focus();
        return;
    } else {
        if (indIncripIngre === 1){
    ...
    numColegiatura = $("#txtNumColegiatura").val();
    ...
    fechaInscripIngre = $("#txtFechaIngresoInscripcion").val();
    var RegExPattern = /^\d{1,2}\/\d{1,2}\/\d{2,4}$/;
    if (fechaInscripIngre === ''){
        mostrarMensaje('danger','[Error: El campo Fecha Inscripcion/Ingreso no puede ser vacio]');
        $("#txtFechaIngresoInscripcion").focus();
        return;
    }
    
    if (fechaInscripIngre.length > 10){
        mostrarMensaje('danger','[Error: El campo Fecha Inscripcion/Ingreso tiene formato incorrecto. Recuerde DD/MM/YYYY]');
        $("#txtFechaIngresoInscripcion").focus();
        return;
    }
    
    if (!RegExPattern.test(fechaInscripIngre)){
        mostrarMensaje('danger','[Error: El campo Fecha Inscripcion/Ingreso tiene formato incorrecto. Recuerde DD/MM/YYYY]');
        $("#txtFechaIngresoInscripcion").focus();
        return;
    }
    var fechaDividida1 = fechaInscripIngre.split('/');
    ...
    var colegiadoResp;
    if (colegiadoRep.esColegiado === 1){
        colegiadoRep.fechaIngreso = fechaInscripIngre;
        colegiadoResp = colegiadoRep.mantPersonaColegiadoInscripIngre(1,1);
    } else {
        ...
        colegiadoRep.numColegiatura = numColegiatura;
        if (colegiadoRep.indExiste === 0){
            colegiadoRep.dni = dni;          
            if (indIncripIngre === 1){
                colegiadoRep.fechaIngreso = fechaInscripIngre;
                colegiadoResp = colegiadoRep.mantPersonaColegiadoInscripIngre(1,1);
            } else {
                colegiadoRep.fechaInscripcion = fechaInscripIngre;
                colegiadoResp = colegiadoRep.mantPersonaColegiadoInscripIngre(1,0);
            }
        } else {
            if (indIncripIngre === 1){
                colegiadoRep.fechaIngreso = fechaInscripIngre;
...
//        if (colegiadoResp.indExiste === 0){
            limpiarTodo();
            deshabilitarDatosColegiado(false);
            habilitarBotonesCrud(true, true, false);
...
function limpiarTodo(){
    ...
    $("#txtFechaIngresoInscripcion").val("");
    $("#txtApellidoPat").val("");
    $("#txtApellidoMat").val("");
    $("#txtNombres").val("");
    $("#txtGrupoSan").val("");
